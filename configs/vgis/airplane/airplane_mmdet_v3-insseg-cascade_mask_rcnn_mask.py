_base_ = '../../../configs/cascade_rcnn/cascade-mask-rcnn_r50_fpn_ms-3x_coco.py'
import platform
...
if platformType == "windows":
    data_root = 'E:\\系统开发\\AI\\AI_Study\\mmdetection_main\\data\\mmdet-insseg\\airplane\\'
else:
...
# 采用 COCO 预训练权重
if platformType == "windows":
    load_from = 'E:\\系统开发\\AI\\AI_Study\\mmdetection_main\\data\\mmdet-insseg\\cascade_mask_rcnn_r50_caffe_fpn_mstrain_3x_coco_20210707_002651-6e29b3a6.pth'
else:
    load_from = './data/mmdet-insseg/cascade_mask_rcnn_r50_caffe_fpn_mstrain_3x_coco_20210707_002651-6e29b3a6'
...
model = dict(
    roi_head=dict(
        bbox_head=[dict(num_classes=num_classes, type='Shared2FCBBoxHead'),
                   ...
                   dict(num_classes=num_classes, type='Shared2FCBBoxHead')], mask_head=dict(num_classes=num_classes)))

# 数据集不同，dataset 输入参数也不一样
# cascase_mask_rcnn的tain的dataset有两层
